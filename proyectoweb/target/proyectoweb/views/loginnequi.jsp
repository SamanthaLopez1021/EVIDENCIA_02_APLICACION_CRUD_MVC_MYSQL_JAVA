<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<!DOCTYPE html>
<html lang="en">
<head>
  
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="assets/css/login1.css"> 
    <script src="https://kit.fontawesome.com/2ee23e2b77.js" crossorigin="anonymous"></script>
    <title>Nequi</title>
</head>
<body class="body" background="assets/img/fondonequi1.png">
    <header>
        <div class="navegador">
          <img src="assets/img/nequi1.png" id="logo">
          <nav>
            <ul>
              <li><a href="index.jsp"><i class="fa-solid fa-house"></i></a></li>
              <li><a href="loginnequi.jsp">Nequi</a></li>
              <li><a href="logindavi.jsp">Daviplata</a></li>
          </ul>
        </nav>
        </div>
      </header>

      <div class="login-page">
        <div class="form">
          <form class="ingresar">
            <h3>NEQUI</h3>
            <input type="text" placeholder="Usuario"/>
            <input type="password" placeholder="Contraseña"/>
            <button><a href="dashboard1.jsp" target="_blank">Ingresar</a></button>
          </form>
        </div>
      </div>
      
    
</body>
</html>